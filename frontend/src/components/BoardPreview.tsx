...
export default function BoardPreview({game}: {game: string}) {
    ...
    return <div className="grid grid-cols-8 w-fit border-black border-8">
        {Array.from({ length: 64}).map((_, i) => <Square index={i} piece={board[i]} key={i} />)}
    </div>    
}
...
function Square({index, piece}: {index: number, piece: string}) {
    ...
    return <div className={cn("size-10", isWhite ? "bg-slate-200" : "bg-amber-700")}>
